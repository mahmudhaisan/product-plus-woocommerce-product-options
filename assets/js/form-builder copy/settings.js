    ...
    const defaultSettings = {
        ...
        advanced: {
            ...
            displayStyle: {
                ...
                fieldBackground: '' // new property for full field background
            }
...
function generateDisplayTab() {
    ...
    // --- Helper to safely access nested style properties ---
    const style = state.currentSettings.advanced?.displayStyle || {};
    const labelStyle = style.label || {};
    const placeholderStyle = style.placeholder || {};
    const defaultValueStyle = style.defaultValue || {};
    const helpTextStyle = style.helpText || {};
    const descriptionStyle = style.description || {};
    const fieldBackground = style.fieldBackground || '#ffffff'; // global wrapper bg

    /**
     * Generates compact style controls for a field part (color, font-size)
     */
    const generateStyleControls = (key, title, currentStyle) => {


        const color = currentStyle.color || '#333333';
        const fontSize = currentStyle.fontSize || '';

        return `
        <div class="mb-3 p-2 border rounded bg-light">
            <label class="wfb-form-label-modern small mb-2 d-block text-dark">${title} Styling</label>
            <div class="d-flex gap-2">
                <div>
                    <label class="small d-block mb-1">Color</label>
                    <input type="color" 
                        class="form-control form-control-color form-control-sm" 
                        id="wfb-style-${key}-color" 
                        value="${color}">
                </div>
                <div>
                    <label class="small d-block mb-1">Font Size (px)</label>
                    <input type="number" 
                        class="form-control form-control-sm" 
                        id="wfb-style-${key}-fontSize" 
                        min="8" step="1" 
    ...
    <div class="wfb-settings-card">
        ...
        <div class="mb-3 p-2 border rounded bg-light">
            <label class="wfb-form-label-modern small mb-2 d-block text-dark">
                Field Background
            </label>
            <input type="color" 
                class="form-control form-control-color form-control-sm" 
                id="wfb-style-fieldBackground" 
        ...
        <div class="wfb-settings-card-body">
            ...
            <!-- Label Styling -->
            ${generateStyleControls('label', 'Label', labelStyle)}
    ...
    if (['text', 'email', 'textarea', 'number', 'url', 'tel', 'password'].includes(state.currentSettings.type)) {
        ...
        <!-- Placeholder Styling -->
        ${generateStyleControls('placeholder', 'Placeholder Text', placeholderStyle)}
        `;

        if (state.currentSettings.type === 'textarea') {
            displayHtml += `
            <div class="mb-3 d-flex gap-3">
                <div style="flex:1;">
                    <label class="wfb-form-label-modern">
                        <i class="fas fa-arrows-alt-v me-1"></i>Rows
                    </label>
                    <input type="number" 
                        class="form-control wfb-form-control-modern" 
                        id="wfb-fieldRows" 
                        min="1"
                        value="${state.currentSettings.rows || 4}">
                </div>
                <div style="flex:1;">
                    <label class="wfb-form-label-modern">
                        <i class="fas fa-ruler-vertical me-1"></i>Height (px)
                    </label>
                    <input type="number" 
                        class="form-control wfb-form-control-modern" 
                        id="wfb-fieldHeight" 
                        min="50"
                        step="10"
                        value="${state.currentSettings.height || 120}">
                </div>
            </div>
            `;
        }
    ...
    displayHtml += `
        ...
        ${generateStyleControls('defaultValue', 'Default Value', defaultValueStyle)}
    `;
    ...
    displayHtml += `
        ...
        <!-- Help Text Styling -->
        ${generateStyleControls('helpText', 'Help Text', helpTextStyle)}
    `;
    ...
    displayHtml += `
        ...
        <!-- Description Styling -->
        ${generateStyleControls('description', 'Field Description', descriptionStyle)}
    `;
...
function generateGeneralPricingSection() {
    ...
    <div class="mb-4">
        <label class="wfb-form-label-modern">
            <i class="fas fa-tag me-1"></i>Additional Pricing
        </label>
        ...
        <div class="form-text mt-2">Set additional pricing for this field. Discount price is optional.</div>
    </div>
    `;
}
...
function collectDisplaySettings() {
    ...
    if (state.currentSettings.type === 'textarea') {
        state.currentSettings.rows = parseInt(jQuery('#wfb-fieldRows').val()) || 4;
        state.currentSettings.height = parseInt(jQuery('#wfb-fieldHeight').val()) || 120;
    }

    // Collect options for select/radio/checkbox
    if (['select', 'radio', 'checkbox'].includes(state.currentSettings.type)) {
        state.currentSettings.options = collectOptionsData();
    }

    // Collect pricing for non-option fields
    if (!['select', 'radio', 'checkbox'].includes(state.currentSettings.type)) {
    ...
    // --- NEW: Collect inline display styles ---
    state.currentSettings.advanced = state.currentSettings.advanced || {};
    state.currentSettings.advanced.displayStyle = {
        label: {
            color: jQuery('#wfb-style-label-color').val() || '',
            fontSize: jQuery('#wfb-style-label-fontSize').val() || ''
        },
        placeholder: {
            color: jQuery('#wfb-style-placeholder-color').val() || '',
            fontSize: jQuery('#wfb-style-placeholder-fontSize').val() || ''
        },
        defaultValue: {
            color: jQuery('#wfb-style-defaultValue-color').val() || '',
            fontSize: jQuery('#wfb-style-defaultValue-fontSize').val() || ''
        },
        helpText: {
            color: jQuery('#wfb-style-helpText-color').val() || '',
            fontSize: jQuery('#wfb-style-helpText-fontSize').val() || ''
        },
        description: {
            color: jQuery('#wfb-style-description-color').val() || '',
            fontSize: jQuery('#wfb-style-description-fontSize').val() || ''
        },
        // Wrapper background for the full field
        fieldBackground: jQuery('#wfb-style-fieldBackground').val() || ''
    };
}
